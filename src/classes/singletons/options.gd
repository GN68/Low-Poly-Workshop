...
func set_option(category: String,name: String, value: Variant) -> void:
	...
	match option.type:
		OPTION_TYPE.BOOLEAN:
			if value is bool: option.data = value
			else: _error(name, "bool")
		OPTION_TYPE.STRING:
			if value is String: option.data = value
			else: _error(name, "String")
		OPTION_TYPE.NUMBER:
			if value is float: option.data = value
			else: _error(name, "float")
		OPTION_TYPE.RANGE:
			if value is float: option.data = value
			else: _error(name, "float")
		OPTION_TYPE.LIST:
			if value is int: option.data = value
			else: _error(name, "int")

	for callback in option.callbacks:
		callback.emit(option.data)